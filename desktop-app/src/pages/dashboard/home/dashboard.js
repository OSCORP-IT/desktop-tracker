...
let activeApps = {}; // To store active applications and their open durations
...
async function takeScreenshot() {
  ...
  try {
    ...
    console.log(`Combined screenshot saved: ${outputScreenshotPath}`);
  ...
  ps.lookup({}, (err, resultList) => {
    ...
    resultList.forEach((process) => {
      ...
      if (!activeApps[appName]) {
        activeApps[appName] = { start: currentTime, duration: 0 };
      } else {
...
function saveAppUsageReport() {
  ...
  for (const [appName, { duration }] of Object.entries(activeApps)) {
