<x-app-layout>
    <x-slot name="page_title">{{ $page_title ?? 'Project Create |' }}</x-slot>
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Project Create</h4>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row mb-2">
                            <form action="{{ url('admin-panel/projects') }}" method="POST" enctype="multipart/form-data" id="project_form">
                                @csrf
                                                    ...
                                                    <div class="team-member">
                                                        <div class="row">
                                                            <div class="col-md-5 mb-1">
                                                                <label for="project_team_members[0][id]" class="form-label">Member <span class="text-danger">*</span></label>
                                                                <select class="form-select @error('project_team_members.0.id') is-invalid @enderror" name="project_team_members[0][id]" required>
                                                                    <option value="">Select Member</option>
                                                                    @foreach ($users as $user)
                                                                        <option value="{{ $user->id }}" {{ old('project_team_members.0.id') == $user->id ? 'selected' : '' }}>{{ $user->name }} (ID: {{ $user->employee_id ?? "N/A" }})</option>
                                                                    @endforeach
                                                                </select>

                                                                @error('project_team_members.0.id')
                                                                    <div class="error">{{ $message }}</div>
                                                                @enderror
                                                            </div>

                                                            <div class="col-md-5 mb-1">
                                                                <label for="project_team_members[0][role]" class="form-label">Role <span class="text-danger">*</span></label>
                                                                <select class="form-select @error('project_team_members.0.role') is-invalid @enderror" name="project_team_members[0][role]" required>
                                                                    @foreach ($project_team_member_roles as $role)
                                                                        <option value="{{ $role }}" {{ old('project_team_members.0.role') == $role ? 'selected' : '' }}>{{ $role }}</option>
                                                                    @endforeach
                                                                </select>

                                                                @error('project_team_members.0.role')
                                                                    <div class="error">{{ $message }}</div>
                                                                @enderror
                                                            </div>
                                            ...
                                            <div class="attachment">
                                                <div class="row">
                                                    <div class="col-md-10 mb-3">
                                                        <label for="attachments[0]" class="form-label">File</label>
                                                    ...
                                                    <div class="col-md-2">
                                                        <label class="form-label">&nbsp;</label>
                                                        <button class="btn remove-btn text-danger" onclick="removeAttachment(this)">Remove</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5 mb-1">
                                ...
                                <select class="form-select" name="project_team_members[${teamMemberIndex}][role]" required>
                                    @foreach (['Member', 'Manager', 'Viewer'] as $role)
            ...
            function addAttachment() {
                ...
                const template = `
                    <div class="attachment">
                        <div class="row">
                            <div class="col-md-10 mb-3">
                                <label for="attachments[${attachmentIndex}]" class="form-label">File <span class="text-danger">*</span></label>
                                <input type="file" class="form-control" name="attachments[${attachmentIndex}]" accept=".jpeg,.png,.jpg,.gif,.pdf,.doc,.docx,.zip" required>
                            </div>
